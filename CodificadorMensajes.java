...
public class CodificadorMensajes
{
    ...
    public void codificarMensaje() 
    {
        if (mensajeCodificado != null) { 
            throw new IllegalStateException("El mensaje ya estÃ¡ codificado"); 
        }
    ...
    public Mensaje obtenerMensajeCodificado() {
        if (mensajeCodificado == null)
            throw new IllegalStateException("Mensaje aÃºn no codificado");
    ...
    //Se crea un arreglo para guardar el codigo de encripcion del tamaño de la cadena
    int codigoEncripcion[] = new int[resultadoFinal.length()];
    System.out.println("");
    System.out.println("Codigo de Encripcion:");
    // Llena el arreglo codigoEncripcion con los dígitos del resto
    ...
    private String encriptarCadena(String str, int[] codigo) {
        if (str == null) // Verifica si la cadena es nula y lanza una excepción si lo es
            throw new IllegalArgumentException("Cadena nula");
        if (codigo == null)  // Verifica si el código es nulo y lanza una excepción si lo es
